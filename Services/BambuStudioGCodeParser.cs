...
public class BambuStudioGCodeParser : IGCodeParser
{
    public bool CanParse(IEnumerable<string> lines)
    {
        foreach (var line in lines)
        {
            if (line.Contains("BambuStudio"))
                return true;
        }
        return false;
    }
    ...
    public GCodeParseResult Parse(IEnumerable<string> lines)
    {
        ...
        int? layerCount = null;
        var regexWeight = new Regex(@";\s*total filament weight \[g\] ?: ?([0-9.]+)", RegexOptions.IgnoreCase);
        var regexLength = new Regex(@";\s*total filament length \[mm\] ?: ?([0-9.]+)", RegexOptions.IgnoreCase);
